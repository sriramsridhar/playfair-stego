...
def Gui():	
	...
	def btn1():
		i = test1.get("1.0","end-1c")
		rtrttext=retr(str(i))
		rtrttext=play_to(rtrttext)
		rtrttext ="Message:: " +rtrttext
		a.config(text=rtrttext)
	def gen():
		t =test1.get("1.0","end-1c")
		opname=t[:-4]+"_out.png"
	...
	test1_label=Label(root,text="Enter Image_name").pack()
	test1=Text(root,height="1")
	test1.pack()
	
	test2_label=Label(root,text="Enter message to encrypt").pack()
	test2=Text(root,height="1")
	test2.pack()
	a = Label(root, text = "Result")
	Button(root, text = "Encrypt",command = btn).pack()
	Button(root, text= "Decrypt",command = btn1).pack() 
	Button(root, text= "Generate Excel sheet",command= gen).pack()
	a.pack()
	root.mainloop()
